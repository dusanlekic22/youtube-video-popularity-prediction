...
if __name__ == '__main__':
    ...
    print(x_train.iloc[:,  np.r_[6:9, 13:15, 17:53]].columns)
    dtrain = xgb.DMatrix(x_train.iloc[:,  np.r_[6:9, 13:15, 17:53]], label=y_train)

    param = {'max_depth': 6, 'eta': 0.3, 'objective': 'binary:logistic', 'nthread': 4, 'eval_metric': ['auc', 'mae', 'rmsle']}

    dtest = xgb.DMatrix(x_test.iloc[:, np.r_[6:9, 13:15, 17:53]], label=y_test)
